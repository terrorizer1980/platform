...
export class TestComponent {
  ...
  calculateAmount() {
    this.showValue =
      this.currentSum == "ATOM" ?
        "$" + (this.inputElement.nativeElement.value * 2.64).toFixed(2)
        : this.inputElement.nativeElement.value;
    // if(this.currentSum == 'ATOM') {
    //   this.showValue = '$'+ (this.inputElement.nativeElement.value * 2.64).toFixed(2);
    // } else if (this.currentSum == 'USD') {
    //   this.showValue = ;
  ...
  stake() {
    // @ts-ignore
    let amount = this.inputElement.nativeElement.value * 1000000;
